    ...
    for (task in tasks) {
      ...
      for (row in 1:nrow(track_pauses)) {
        if (row == 1) {
          ...
          X7_prev_articulation_rate = append(X7_prev_articulation_rate, 0)
          next
        }
        ...
        X7_prev_articulation_rate = append(X7_prev_articulation_rate, nrow(syllables_so_far) / sum(speech_time_so_far$duration))
      }
    }
  }
}
...
pauses$X7_prev_articulation_rate <- as.numeric(X7_prev_articulation_rate)
...
· bs, which specifies the type of spline function. 
The default is bs = "tp", yielding a thin-plate spline basis. A range of other options are available, e.g., by specifying bs = "cr" yields a cubic spline basis. By specifying bs = "re" and a factor variable as the predictor, we obtain a random effect estimated with respect to the factor variable. 
  ...
  for (language in languages) {
    for (task in tasks) {
      ...
      plt <- ggplot(testData, aes(x = tmin, y = Predictions)) +
        ...
        labs(x = "Actual Pause Duration", y = "Predicted Pause Duration", title = "Actual vs Predicted Pause Durations") +
        theme_minimal()
      print(plt)
...
X1_prev_pause_duration: Duration of the previous pause. A significant predictor in L2 models, suggesting that the length of the preceding pause can have more influence on the current pause duration rather than L1.

X3_prev_speech_complexity: Complexity of the preceding speech segment. This predictor showed significance only in L2 task 2, implying that more complex speech segments might influence the subsequent pause duration.

X5_prev_pause_type: Type of the previous pause (silent or filled). Generally, this categorical predictor did not show a consistent significant effect across tasks and languages, suggesting that the type of previous pause doesn’t have a strong direct impact on the duration of the current pause.

X6_prev_speech_rate & X7_prev_articulation_rate: This predictor were significant in all languages and tasks, indicating that faster or slower speech rates and articulation rate before a pause can affect the length of the pause. 

TransitionPoint: The transition between speech and pause. Its significance across all models shows the complex dynamics at play in speech-to-pause transitions.

PauseType: The type of the current pause (silent or filled). This predictor was significant in all models, indicating that different types of pauses inherently have different durations.

The variation in significance of predictors across tasks and languages shows that the dynamics of speech and pause are influenced by a complex interplay of factors including the cognitive and linguistic demands of the task, the language spoken, and the specific characteristics of the preceding speech and pauses.
  ...
  for (language in languages) {
    for (task in tasks) {
      ...
      rf_model <- randomForest(PauseDuration ~ X1_prev_pause_duration + 
                                                ...
                                                X3_prev_speech_complexity +
                                                X4_prev_speech_syllables +
                                                X5_prev_pause_type +
                                                X6_prev_speech_rate +
                                                X7_prev_articulation_rate +
                                                TransitionPoint +
                                                PauseType,
                                data = trainData, 
                                ntree = 500) 
      ...
      # Plot 2: Actual vs Predicted Pause Durations
      plt <- ggplot(testData, aes(x = tmin, y = Predictions)) +
        geom_point(alpha = 0.5) +
        geom_line(aes(x = tmin, y = PauseDuration), color = "red") + 
...
# Delete the code below to NOT filter
# 
# all_participants <- sort(unique(c(pauses$ParticipantID)))
# # all_participants <- c(SELECTED_PARTICIPANT_ID)
# languages <- c("L1", "L2")
  ...
  for (language in languages) {
    for (task in tasks) {
      ...
      gam_model <- gam(PauseDuration ~ s(X1_prev_pause_duration) + 
                                         s(X2_prev_speech_duration) + 
                                         s(X3_prev_speech_complexity) +
                                         s(X4_prev_speech_syllables, k = 4) +
                                         factor(X5_prev_pause_type) +
                                         s(X6_prev_speech_rate) +
                                         s(X7_prev_articulation_rate) +
                                         s(tmin) +
                                         X2_prev_speech_duration:X4_prev_speech_syllables +
                                         PauseType
                       , data = trainData)
      ...
      plt <- ggplot(testData, aes(x = tmin, y = Predictions)) +
        geom_point(alpha = 0.5) +
        geom_line(aes(x = tmin, y = PauseDuration), color = "red") +  # Adds a 45 degree line for reference
        labs(x = "Actual Pause Duration", y = "Predicted Pause Duration", title = "Actual vs Predicted Pause Durations") +
        theme_minimal()
      print(plt)
...
library(caret) # For R^2 and data splitting

# Assuming 'pauses' is your dataset
all_participants <- c(SELECTED_PARTICIPANT_ID)
languages <- c("L1", "L2")
tasks <- c("Task1", "Task2")
...
for (participant in all_participants) {
  for (language in languages) {
    for (task in tasks) {
      ...
      # Fitting Random Forest model on the training set
      rf_model <- randomForest(PauseDuration ~ X1_prev_pause_duration + 
                                                X2_prev_speech_duration + 
                                                X3_prev_speech_complexity +
                                                X4_prev_speech_syllables +
...
for (participant in all_participants) {
  ...
  for (language in languages) {
    for (task in tasks) {
      # Actual vs Predicted Plot
      plt_data <- testData %>% filter(Language == language, Task == task)
      plt <- ggplot(plt_data, aes(x = tmin, y = Predictions)) +
        geom_point(alpha = 0.5) +
        geom_line(aes(x = tmin, y = PauseDuration), color = "red") +  # Adds a 45 degree line for reference
        labs(x = "Actual Pause Duration", y = "Predicted Pause Duration", title = "Actual vs Predicted Pause Durations") +
        theme_minimal()

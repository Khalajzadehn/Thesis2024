...
X8_next_600_msec_complexity = c()
X9_prev_600_msec_complexity = c ()
    ...
    for (task in tasks) {
      ...
      for (row in 1:nrow(track_pauses)) {
        if (row == 1) {
          ...
          X8_next_600_msec_complexity = append(X8_next_600_msec_complexity, 0)
          X9_prev_600_msec_complexity = append(X9_prev_600_msec_complexity, 0)
          next
        }
        ...
        X8_next_600_msec_complexity = append(X8_next_600_msec_complexity, ifelse(nrow(wordfreq_next_600_msec) > 0, max(as.numeric(wordfreq_next_600_msec$text)), 0))

        wordfreq_prev_600_msec = track_wordfreq %>% filter(tmin < track_pauses[row,]$tmax, tmin > track_pauses[row,]$tmax - 0.6)
        X9_prev_600_msec_complexity = append(X9_prev_600_msec_complexity, ifelse(nrow(wordfreq_prev_600_msec) > 0, max(as.numeric(wordfreq_prev_600_msec$text)), 0))
      }
    }
  }
}
...
pauses$X8_next_600_msec_complexity <- as.numeric(X8_next_600_msec_complexity)
pauses$X9_prev_600_msec_complexity <- as.numeric(X9_prev_600_msec_complexity)
...
# Adding new predictor next 6 sec word complexity
  ...
  for (language in languages) {
    for (task in tasks) {
      ...
      gam_model <- gam(PauseDuration ~ s(X1_prev_pause_duration) + 
                                         ...
                                         factor(X5_prev_pause_type) +
                                         s(X6_prev_speech_rate) +
                                         s(X8_next_600_msec_complexity, k = 4) +
                                         s(tmin) +
                                         PauseType
                       ,data = trainData)
      ...
      plt <- ggplot(testData, aes(x = tmin, y = Predictions)) +
        ...
        geom_segment(aes(xend = tmin, y = Predictions, yend = PauseDuration, color = errorType), size = 2, alpha = 0.5) +  # Error lines
        labs(x = "time (s)", y = "Pause Duration (Actual vs Predicted)", title = "Actual vs Predicted Pause Durations") +
        theme_gray() +
        scale_color_manual(values = c("Filled" = "purple", "Silent" = "blue", "Positive" = "green", "Negative" = "red"))  # Optional: manually specify colors for each PauseType
